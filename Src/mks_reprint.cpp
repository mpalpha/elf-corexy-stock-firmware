...
void mkstft_ui_load()
{
	...
	//if(valid_flag == BAK_INF_VALID_FLAG)
	{
		...
		epr_read_data(EPR_ENABLE_LED,(uint8_t *)&gCfgItems.enable_led,sizeof(gCfgItems.enable_led));
		epr_read_data(EPR_ENABLE_OFF,(uint8_t *)&gCfgItems.enable_off,sizeof(gCfgItems.enable_off));
...
void mkstft_ui_set_epr()
{
	...
	if(valid_flag != BAK_INF_VALID_FLAG)
	{
		...
		epr_write_data(EPR_ENABLE_LED,(uint8_t *)&gCfgItems.enable_led,sizeof(gCfgItems.enable_led));
		epr_write_data(EPR_ENABLE_OFF,(uint8_t *)&gCfgItems.enable_off,sizeof(gCfgItems.enable_off));
...
void mkstft_ui_init()
{
		...
		gCfgItems.enable_led=0;
		gCfgItems.enable_off=1;
...
void mks_home_moveXY(float X,float Y,float speed)
{
      char string[20];
    //移动打印头X,Y位置
	memset(mksReprint.command_queue,0,MAX_CMD_SIZE);
	...
	strcat(mksReprint.command_queue," Y");
	memset(string,0,sizeof(string));
	sprintf(string,"%.3f F%d",Y,(int)speed);
	strcat(mksReprint.command_queue,string);
...
void mks_contiuePrintPwdwn()
{
	...
	HAL_GPIO_WritePin(E1_ENA_GPIO_Port, E1_ENA_Pin, GPIO_PIN_RESET);
