...
SD_Error SD_Init(void)
{
  ...
  RCC->AHBENR|=1<<10;    	//使能SDIO时钟	   	 
	...
	SDIO->MASK=0x00000000;	
 	...
 	//MY_NVIC_Init(0,0,SDIO_IRQChannel,2);//SDIO中断配置
  ...
  for(i=0;i<74;i++)
	...
	{																  
		...
		{	   										   
			...
			SDIO_Send_Cmd(SD_CMD_SD_APP_OP_COND,1,SD_VOLTAGE_WINDOW_SD|SD_OCR_XPC|SD_OCR_S18 |SDType);//发送ACMD41,短响应	 
 	...
 	}else//MMC卡
	{
  	...
  	return(errorstatus);
		#else
		SDIO_Send_Cmd(SD_CMD_GO_IDLE_STATE,0,0xF0F0F0F0);//发送进入IDLE STAGE模式命令.												  
		errorstatus=CmdError();
		if(errorstatus!=SD_OK)return errorstatus;				
		...
		while ((!validvoltage) && (count < SD_MAX_VOLT_TRIAL))
		{
			SDIO_Send_Cmd(SD_CMD_SEND_OP_COND,1,SD_VOLTAGE_WINDOW_MMC | MMC_HIGH_CAPACITY);//发送CMD0,短响应	 
			errorstatus=CmdResp3Error(); 					//等待R3响应   
 			if(errorstatus!=SD_OK)return errorstatus;   	//响应错误  
			response=SDIO->RESP1;			   				//得到响应
			validvoltage=(((response>>31)==1)?1:0);
...
SD_Error SD_InitializeCards(void)
{
	...
	if((SDIO_STD_CAPACITY_SD_CARD_V1_1==CardType)||
		...
		(SDIO_HIGH_CAPACITY_MMC_CARD  == CardType) )//判断卡类型
	{
...
SD_Error SD_EnableWideBusOperation(u32 wmode)
{
 	...
 	{
		if(wmode>=2)return SD_UNSUPPORTED_FEATURE;//MMC卡不支持
		else
		{
...
//addr:读取地址
//blksize:块大小
SD_Error SD_ReadBlock(u8 *buf,uint64_t addr,u16 blksize)
{	  
	SD_Error errorstatus=SD_OK;
	u8 power;
...
//nblks:要读取的块数
//返回值:错误状态
SD_Error SD_ReadMultiBlocks(u8 *buf,uint64_t addr,u16 blksize,u32 nblks)
{
  	SD_Error errorstatus=SD_OK;
	u8 power;
   	u32 count=0,*tempbuff=(u32*)buf;//转换为u32指针
...
//blksize:块大小	  
//返回值:错误状态
SD_Error SD_WriteBlock(u8 *buf,uint64_t addr,  u16 blksize)
{
	SD_Error errorstatus = SD_OK;
...
//nblks:要写入的块数
//返回值:错误状态												   
SD_Error SD_WriteMultiBlocks(u8 *buf,uint64_t addr,u16 blksize,u32 nblks)
{
	SD_Error errorstatus = SD_OK;
	u8  power = 0, cardstate = 0;
	u32 timeout=0,bytestransferred=0;
...
//SDIO中断服务函数
extern volatile uint8_t sd_pause_flag;
...
{	
  ...
  DMA2_Channel4->CCR&=~(0X7FF<<4);	//清除之前的设置,DIR,CIRC,PINC,MINC,PSIZE,MSIZE,PL,MEM2MEM
	...
	DMA2_Channel4->CCR|=3<<12;//2<<12; 			//高优先级	  
...
u8 SD_ReadDisk(u8*buf,u32 sector,u8 cnt)
{
	...
	if((u32)buf%4!=0)
	{
	 	for(n=0;n<cnt;n++)
		{
		 	sta=SD_ReadBlock(SDIO_DATA_BUFFER,(uint64_t)sector*512UL,512);    	//单个sector的读操作
			memcpy(buf,SDIO_DATA_BUFFER,512);
			buf+=512;
		} 
	}else
	{
		if(cnt==1)sta=SD_ReadBlock(buf,(uint64_t)sector*512UL,512);    	//单个sector的读操作
		else sta=SD_ReadMultiBlocks(buf,(uint64_t)sector*512UL,512,cnt);//多个sector  
	}
	return sta;
}
//写SD卡
	...
	if((u32)buf%4!=0)
	{
	 	for(n=0;n<cnt;n++)
		{
			memcpy(SDIO_DATA_BUFFER,buf,512);
		 	sta=SD_WriteBlock(SDIO_DATA_BUFFER,(uint64_t)sector*512UL,512);    	//单个sector的写操作
			buf+=512;
		} 
	}else
	{
		if(cnt==1)sta=SD_WriteBlock(buf,(uint64_t)sector*512UL,512);    	//单个sector的写操作
		else sta=SD_WriteMultiBlocks(buf,(uint64_t)sector*512UL,512,cnt);	//多个sector  
	}
	return sta;
}
