...
void Stepper::StepperHandler() {
  ...
  for (uint8_t i = step_loops; i--;) {
    ...
    #define PULSE_START(AXIS) \
      ...
      if (_COUNTER(AXIS) > 0) { _APPLY_STEP(AXIS)(!_INVERT_STEP_PIN(AXIS),0); for(volatile int i_axis=0;i_axis<5;i_axis++);}
    ...
    #define PULSE_STOP(AXIS) \
      if (_COUNTER(AXIS) > 0) { \
        ...
        _APPLY_STEP(AXIS)(_INVERT_STEP_PIN(AXIS),0); for(volatile int i_axis=0;i_axis<20;i_axis++);\
      }

    // Advance the Bresenham counter; start a pulse if the axis needs a step
    #define Z_PULSE_START(AXIS) \
      _COUNTER(AXIS) += current_block->steps[_AXIS(AXIS)]; \
      if (_COUNTER(AXIS) > 0) { Z_APPLY_STEP(!_INVERT_STEP_PIN(AXIS),0);for(volatile int iz_axis=0;iz_axis<5;iz_axis++);}
    ...
    #define Z_PULSE_STOP(AXIS) \
      if (_COUNTER(AXIS) > 0) { \
        _COUNTER(AXIS) -= current_block->step_event_count; \
        count_position[_AXIS(AXIS)] += count_direction[_AXIS(AXIS)]; \
        Z_APPLY_STEP(_INVERT_STEP_PIN(AXIS),0); for(volatile int i_axis=0;i_axis<5;i_axis++);\
      }
...
void Stepper::finish_and_disable_noZ() {
  synchronize();
  disable_noZ_steppers();
}
