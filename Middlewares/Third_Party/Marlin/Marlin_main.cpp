...
inline void get_serial_commands() {
  ...
  // send "wait" to indicate Marlin is still waiting.
    ...
    if (commands_in_queue == 0 && !MYSERIAL.available() && ELAPSED(ms, last_command_time + NO_TIMEOUTS)) {
	  ...
	  /*5s?¨?2a2?|ì?????¨2¨oy?Y?¨°¨¨??a??¨?D¨￠a?¨2?????ê??¨|¨o1¨??wifi*/
      ...
      if (card_eof || n == -1
        ...
        if (!sd_comment_mode) 
        {
			...
			if(sd_char == 0x5a ) //"Z"
			{
				...
				#if 1
				mksReprint.sdpos_bak = mksReprint.sdpos;//sdpos_bakμ?èY??μ?￡????′ê1ó?
				if(gCfgItems.pwroff_save_mode != 1)//sd?¨±￡′???μ?êy?Y
                #endif
...
inline void gcode_G0_G1(
  ...
  if (IsRunning() && G0_G1_CONDITION) {
   ...
   /*    
    ...
    {
        if ((destination[2] <= gCfgItems.breakpoint_z_pos)&&(card.sdpos >= 5000)) //断点续打需要大于5k之后的内容才能进行续打。
        {
            return;
        }
    }
  ...
  else
  {
	...
	//if(home_all)
	  //{
		...
		//mks_home_moveZ(5);
	      //移动X,Y
		//mks_home_moveXY(mksReprint.mks_zero_x,mksReprint.mks_zero_y,mksCfg.homing_feedrate_xy);
		  //mks_home_moveZ(0);
		  //SYNC_PLAN_POSITION_KINEMATIC();
	  //}
}
...
void disable_e_steppers() {
  disable_E0();
  if(!Z_DUAL_STEPPER_DRIVERS)disable_E1();
  disable_E2();
  disable_E3();
  disable_E4();
}
