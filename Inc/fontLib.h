/* SimSun_27.c - SimSun_27 font for UCGUI font driver */ 
/********************************************************************************************************
*                                            UCGUI
*                                       http://www.ucgui.com
*
*                             (c) Copyright 2005-2006, UCGUI专业论坛, 学习交流UCGUI
*
*                               	ucgui@163.com 2005.07.31
*                                           作者: ucgui
*********************************************************************************************************
*/
//字体:Terminal  15号粗体
/*0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ板绑备标厂出打底调动定度法繁返复关恢回挤加简减进冷料目喷却热日少设体头退未温文言移印于语预在增整正增整正置中准零上下一页风扇开速少单双普通三角洲默认最新暂停止床操作变确常快出时间件系统触屏校正主板本地盘总量载入选择机型连接闭高慢无请点击心点完成全半闭检查取消找不到卡插重试线网络密码端口更多状态已+-_>''!~:;\/%., ?*/
#ifndef _FONT_LIB_H_
#define _FONT_LIB_H_

#include "gui.h" 
#ifndef GUI_FLASH 
	#define GUI_FLASH 
#endif 
extern GUI_FLASH const GUI_FONT GUI_FontHZ_fontHz14;
extern GUI_FLASH const GUI_FONT GUI_FontHZ_fontHz16;
extern GUI_FLASH const GUI_FONT GUI_FontHZ_fontHz18;


#endif
