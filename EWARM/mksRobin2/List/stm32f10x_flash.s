///////////////////////////////////////////////////////////////////////////////
//
// IAR ANSI C/C++ Compiler V7.10.1.6676/W32 for ARM       25/Feb/2020  18:23:19
// Copyright 1999-2014 IAR Systems AB.
//
//    Cpu mode     =  thumb
//    Endian       =  little
//    Source file  =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_ºŒ∫Ã»˝Œ¨\mksRobin_Nano_code\Src\stm32f10x_flash.c
//    Command line =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_ºŒ∫Ã»˝Œ¨\mksRobin_Nano_code\Src\stm32f10x_flash.c
...
//    List file    =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_ºŒ∫Ã»˝Œ¨\mksRobin_Nano_code\EWARM\mksRobin2/List\stm32f10x_flash.s
//
///////////////////////////////////////////////////////////////////////////////
...
//  469     FLASH->CR|= CR_PER_Set;
        LDR.W    R4,??DataTable25_4  ;; 0x40022010
        LDR      R0,[R4, #+0]
        ORR      R0,R0,#0x2
        STR      R0,[R4, #+0]
...
//  633     FLASH->OPTKEYR = FLASH_KEY1;
        LDR.N    R5,??DataTable25_5  ;; 0x40022008
        LDR.N    R0,??DataTable25_2  ;; 0x45670123
        STR      R0,[R5, #+0]
...
//  650       OB->RDP = (uint16_t)rdptmp; 
        LDR.N    R0,??DataTable25_6  ;; 0x1ffff800
        STRH     R4,[R0, #+0]
...
//  818     FLASH->CR |= CR_PG_Set;
        LDR.N    R5,??DataTable25_4  ;; 0x40022010
        LDR      R0,[R5, #+0]
        ORR      R0,R0,#0x1
        STR      R0,[R5, #+0]
//  819   
...
//  905     FLASH->CR |= CR_PG_Set;
        LDR.N    R6,??DataTable25_4  ;; 0x40022010
        LDR      R0,[R6, #+0]
        ORR      R0,R0,#0x1
        STR      R0,[R6, #+0]
//  906   
...
//  939     FLASH->OPTKEYR = FLASH_KEY1;
        LDR.N    R4,??DataTable25_5  ;; 0x40022008
        LDR.N    R0,??DataTable25_2  ;; 0x45670123
        STR      R0,[R4, #+0]
...
//  996     FLASH->OPTKEYR = FLASH_KEY1;
        LDR.W    R8,??DataTable25_5  ;; 0x40022008
...
// 1001       OB->WRP0 = WRP0_Data;
        LDR.N    R0,??DataTable25_7  ;; 0x1ffff808
        STRH     R4,[R0, #+0]
// 1002       
...
// 1139     OB->USER = OB_IWDG | (uint16_t)(OB_STOP | (uint16_t)(OB_STDBY | ((uint16_t)0xF8))); 
        ...
        ORRS     R0,R4,R0
        ORR      R0,R0,#0xF8
        LDR.N    R1,??DataTable25_8  ;; 0x1ffff802
        STRH     R0,[R1, #+0]
// 1140   
...
FLASH_GetUserOptionByte:
        LDR.N    R0,??DataTable25_9  ;; 0x4002201c
        LDR      R0,[R0, #+0]
        LSRS     R0,R0,#+2
        BX       LR               ;; return
...
FLASH_GetWriteProtectionOptionByte:
        LDR.N    R0,??DataTable25_10  ;; 0x40022020
        LDR      R0,[R0, #+0]
        BX       LR               ;; return
...
// 1403   if(FLASH_FLAG == FLASH_FLAG_OPTERR) 
        LDR.N    R2,??DataTable25_11  ;; 0x4002200c
        CMP      R0,#+1
...
// 1434   *         - For STM32F10X_XL devices, this function clears Bank1 or Bank2ís pending flags
// 1435   *         - For other devices, it clears Bank1ís pending flags.
...
FLASH_ClearFlag:
        LDR.N    R1,??DataTable25_11  ;; 0x4002200c
        STR      R0,[R1, #+0]
...
// 1488     { 
// 1489       flashstatus = FLASH_ERROR_PG2;
        MOVS     R0,#+2
        BX       LR
// 1490     }
// 1491     else 
...
// 1494       {
// 1495         flashstatus = FLASH_ERROR_WRP2;
        MOVS     R0,#+3
// 1496       }
// 1497       else
// 1498       {
...
// 1526     { 
// 1527       flashstatus = FLASH_ERROR_PG2;
        MOVS     R0,#+2
        BX       LR
// 1528     }
// 1529     else 
...
// 1532       {
// 1533         flashstatus = FLASH_ERROR_WRP2;
        MOVS     R0,#+3
// 1534       }
// 1535       else
// 1536       {
...
// 1564     { 
// 1565       flashstatus = FLASH_ERROR_PG2;
// 1566     }
// 1567     else 
// 1568     {
// 1569       if((FLASH->SR2 & (FLASH_FLAG_BANK2_WRPRTERR & 0x7FFFFFFF)) != 0 )
// 1570       {
// 1571         flashstatus = FLASH_ERROR_WRP2;
// 1572       }
// 1573       else
// 1574       {
