///////////////////////////////////////////////////////////////////////////////
//
// IAR ANSI C/C++ Compiler V7.10.1.6676/W32 for ARM       25/Feb/2020  18:22:45
// Copyright 1999-2014 IAR Systems AB.
//
//    Cpu mode     =  thumb
//    Endian       =  little
//    Source file  =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_¼ÎºÌÈýÎ¬\mksRobin_Nano_code\User\ui\draw_filamentchange.cpp
//    Command line =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_¼ÎºÌÈýÎ¬\mksRobin_Nano_code\User\ui\draw_filamentchange.cpp
...
//    List file    =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_¼ÎºÌÈýÎ¬\mksRobin_Nano_code\EWARM\mksRobin2/List\draw_filamentchange.s
//
///////////////////////////////////////////////////////////////////////////////
...
//   17 
//   18 //extern TFT_FIFO gcodeCmdTxFIFO;		//gcode Ö¸Áî·¢ËÍ¶ÓÁÐ
//   19 //extern TFT_FIFO gcodeCmdRxFIFO;		//gcode	Ö¸Áî½ÓÊÕ¶ÓÁÐ
...
//  311 
//  312 	disp_filament_sprayer_temp();
...
//  313 	gCfgItems.curSprayerChoose = active_extruder;
        ...
        LDRSB    R0,[R0, #+0]
        STRB     R0,[R4, #+156]
//  314 	disp_filament_sprayer_type();
        ADD      SP,SP,#+24
          CFI CFA R13+32
        POP      {R4-R10,LR}
          CFI R4 SameValue
...
//  315 	//TEXT_SetTextColor(textExtruMsg, gCfgItems.state_text_color);
//  316 	//TEXT_SetBkColor(textExtruMsg, gCfgItems.state_background_color);
//  317 	
//  318 	//sprintf((char*)buf,filament_menu.ready_replace);
//  319 	//TEXT_SetText(textExtruMsg, (const char *)buf);	
//  320 }
//  321 
//  322 
        ...
        THUMB
//  323 void disp_filament_sprayer_temp()
//  324 {
...
//  325 	int8_t buf[30] = {0};
        ...
        MOVS     R1,#+32
          CFI FunCall __aeabi_memclr4
        BL       __aeabi_memclr4
//  326 	int8_t buf1[20] = {0};
        ADD      R0,SP,#+0
        MOVS     R1,#+20
          CFI FunCall __aeabi_memclr4
        BL       __aeabi_memclr4
//  327 
//  328 	TEXT_SetTextColor(textExtruTemp, gCfgItems.state_text_color);
...
//  329 	TEXT_SetBkColor(textExtruTemp, gCfgItems.state_background_color);
        LDR      R1,[R5, #+12]
        LDRSH    R0,[R4, #+6]
          CFI FunCall TEXT_SetBkColor
        BL       TEXT_SetBkColor
//  330 	//TEXT_SetTextAlign(textExtruTemp,  GUI_TA_VERTICAL| GUI_TA_HCENTER);
//  331 	sprintf((char*)buf,"E%d: ",gCfgItems.curSprayerChoose+1);
        LDRSB    R0,[R5, #+156]
        ADDS     R2,R0,#+1
        ...
        ADD      R0,SP,#+20
          CFI FunCall sprintf
        BL       sprintf
//  332 	sprintf((char*)buf1, filament_menu.stat_temp, (int)thermalManager.current_temperature[gCfgItems.curSprayerChoose],(int)thermalManager.target_temperature[gCfgItems.curSprayerChoose]);
        LDRSB    R0,[R5, #+156]
        ...
        ADD      R0,SP,#+0
          CFI FunCall sprintf
        BL       sprintf
//  333 	strcat((char*)buf,(char*)buf1);
        ADD      R1,SP,#+0
        ADD      R0,SP,#+20
          CFI FunCall strcat
        BL       strcat
//  334 	TEXT_SetText(textExtruTemp, (const char *)buf);
        ADD      R1,SP,#+20
        LDRSH    R0,[R4, #+6]
...
//  336 
        ...
        THUMB
//  337 void disp_filament_sprayer_type()
//  338 {
_Z26disp_filament_sprayer_typev:
        ...
        SUB      SP,SP,#+8
          CFI CFA R13+32
//  339 	if(gCfgItems.curSprayerChoose == 0)
...
//  340 	{			
//  341 		#if VERSION_WITH_PIC
//  342 		BUTTON_SetBmpFileName(buttonSprayType.btnHandle, "bmp_extru1.bin",1);
        MOVS     R2,#+1
        ADR.W    R1,`?<Constant "bmp_extru1.bin">`
          CFI FunCall BUTTON_SetBmpFileName
        BL       BUTTON_SetBmpFileName
//  343 		BUTTON_SetBitmapEx(buttonSprayType.btnHandle, 0, &bmp_struct, BMP_PIC_X, BMP_PIC_Y);
        LDRSB    R0,[R8, #+0]
        STR      R0,[SP, #+0]
        LDRSB    R3,[R7, #+0]
        MOV      R2,R6
...
//  346 	else
//  347 	{
//  348 		#if VERSION_WITH_PIC
//  349 		BUTTON_SetBmpFileName(buttonSprayType.btnHandle, "bmp_extru2.bin",1);
??disp_filament_sprayer_type_0:
        MOVS     R2,#+1
        ADR.W    R1,`?<Constant "bmp_extru2.bin">`
          CFI FunCall BUTTON_SetBmpFileName
        BL       BUTTON_SetBmpFileName
//  350 		BUTTON_SetBitmapEx(buttonSprayType.btnHandle, 0, &bmp_struct, BMP_PIC_X, BMP_PIC_Y);
        LDRSB    R0,[R8, #+0]
        STR      R0,[SP, #+0]
        LDRSB    R3,[R7, #+0]
        MOV      R2,R6
...
//  354 	{
//  355 		if(gCfgItems.curSprayerChoose == 0)
        LDRSB    R0,[R5, #+156]
        CMP      R0,#+0
        BNE.N    ??disp_filament_sprayer_type_3
//  356 		{
//  357 			BUTTON_SetText(buttonSprayType.btnHandle,filament_menu.ext1);
        LDR.N    R0,??DataTable17_18
        LDR      R1,[R0, #+12]
        LDRSH    R0,[R4, #+0]
        POP      {R2-R8,LR}
        ...
        B.W      BUTTON_SetText
          ...
          CFI CFA R13+32
//  358 		}
//  359 		else if(gCfgItems.curSprayerChoose == 1)
??disp_filament_sprayer_type_3:
        LDRSB    R0,[R5, #+156]
        CMP      R0,#+1
        BNE.N    ??disp_filament_sprayer_type_2
//  360 		{
//  361 			BUTTON_SetText(buttonSprayType.btnHandle,filament_menu.ext2);
        LDR.N    R0,??DataTable17_18
        LDR      R1,[R0, #+16]
        LDRSH    R0,[R4, #+0]
        POP      {R2-R8,LR}
...
//  365 #if 0
//  366 void FilamentChange_handle()
//  367 {
//  368 	char buf[15] = {0};
//  369 
//  370 	switch(filamentchange_Process)
//  371 	{
//  372 	case 1:
//  373 
//  374 		if(gcodeCmdTxFIFO.count <= 12)
//  375 		{
//  376 			//pushFIFO(&gcodeCmdTxFIFO, RELATIVE_COORD_COMMAN);
//  377 			
//  378 			if(gCfgItems.sprayerNum == 2)
//  379 			{							
//  380 				sprintf(buf,"T%d\n",gCfgItems.curSprayerChoose);
//  381 				pushFIFO(&gcodeCmdTxFIFO, (unsigned char *)buf);
//  382 				memset(buf,0,sizeof(buf));
//  383 				sprintf(buf, "G1 E%d F%d\n", gCfgItems.filamentchange_step, gCfgItems.filamentchange_speed);
//  384 				pushFIFO(&gcodeCmdTxFIFO, (unsigned char *)buf);	
//  385 			}
//  386 			else
//  387 			{
//  388 				MOVE_E_COMMAN(buf, gCfgItems.filamentchange_step, gCfgItems.filamentchange_speed);
//  389 				pushFIFO(&gcodeCmdTxFIFO, (unsigned char *)buf);
//  390 			} 	
//  391 
//  392 			//pushFIFO(&gcodeCmdTxFIFO, ABSOLUTE_COORD_COMMAN);		
//  393 		}
//  394 
//  395 		break;
//  396 	case 2:
//  397 	
//  398 		if(gcodeCmdTxFIFO.count <= 12)
//  399 		{
//  400 			//pushFIFO(&gcodeCmdTxFIFO, RELATIVE_COORD_COMMAN);
//  401 					
//  402 			if(gCfgItems.sprayerNum == 2)
//  403 			{							
//  404 				sprintf(buf,"T%d\n",gCfgItems.curSprayerChoose);
//  405 				pushFIFO(&gcodeCmdTxFIFO, (unsigned char *)buf);
//  406 				memset(buf,0,sizeof(buf));
//  407 				sprintf(buf, "G1 E%d F%d\n", 0-gCfgItems.filamentchange_step, gCfgItems.filamentchange_speed);
//  408 				pushFIFO(&gcodeCmdTxFIFO, (unsigned char *)buf);	
//  409 			}
//  410 			else
//  411 			{
//  412 				MOVE_E_COMMAN(buf, 0-gCfgItems.filamentchange_step, gCfgItems.filamentchange_speed);
//  413 				pushFIFO(&gcodeCmdTxFIFO, (unsigned char *)buf);
//  414 			} 	
//  415 
//  416 			//pushFIFO(&gcodeCmdTxFIFO, ABSOLUTE_COORD_COMMAN);			
//  417 		}
//  418 		break;
//  419 	case 3:
//  420 		initFIFO(&gcodeCmdTxFIFO);
//  421 		filamentchange_Process = 0;	
//  422 		pushFIFO(&gcodeCmdTxFIFO, (unsigned char *)ABSOLUTE_COORD_COMMAN);//È·±£ºóÐøÒÔ¾ø¶Ô×ø±êÖ´ÐÐ
//  423 		//ÒÆ¶¯ºóÂíÉÏ±£´æÊý¾Ý
//  424 		if(last_disp_state == PRINT_MORE_UI)
//  425 			positionSaveFlag = 1;		
//  426 		break;
//  427 	default:break;		
//  428 	}
//  429 }
...
Clear_FilamentChange:
        PUSH     {R4,LR}
          ...
          CFI R4 Frame(CFA, -8)
          CFI CFA R13+8
//  433 	GUI_SetBkColor(gCfgItems.background_color);
        LDR.N    R0,??DataTable17_4
        LDR      R0,[R0, #+4]
          CFI FunCall GUI_SetBkColor
        BL       GUI_SetBkColor
//  434 	if(WM_IsWindow(hFilamentChangeWnd))
        LDR.N    R4,??DataTable17_7
        LDRSH    R0,[R4, #+4]
...
//  435 	{
//  436 		WM_DeleteWindow(hFilamentChangeWnd);
        LDRSH    R0,[R4, #+4]
