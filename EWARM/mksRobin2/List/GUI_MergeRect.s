...
//    Endian       =  little
//    Source file  =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_¼ÎºÌÈýÎ¬\mksRobin_Nano_code\Middlewares\Ucgui\GUI\Core\GUI_MergeRect.c
//    Command line =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_¼ÎºÌÈýÎ¬\mksRobin_Nano_code\Middlewares\Ucgui\GUI\Core\GUI_MergeRect.c
...
//    List file    =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_¼ÎºÌÈýÎ¬\mksRobin_Nano_code\EWARM\mksRobin2/List\GUI_MergeRect.s
//
///////////////////////////////////////////////////////////////////////////////
...
//   42 *
//   43 * Purpose:
//   44 *   Calc smalles rectangles containing both rects.
//   45 */

        SECTION `.text`:CODE:NOROOT(1)
          CFI Block cfiBlock0 Using cfiCommon0
          CFI Function GUI_MergeRect
        THUMB
//   46 void GUI_MergeRect(GUI_RECT* pDest, const GUI_RECT* pr0, const GUI_RECT* pr1) {
//   47   if (pDest) {
GUI_MergeRect:
        CMP      R0,#+0
        BNE.N    ??GUI_MergeRect_0
        BX       LR
//   48     if (pr0 && pr1) {
??GUI_MergeRect_0:
        PUSH     {R3,R4}
          ...
          CFI CFA R13+8
        CMP      R1,#+0
        BEQ.N    ??GUI_MergeRect_1
        CMP      R2,#+0
        BEQ.N    ??GUI_MergeRect_1
//   49       pDest->x0 = Min(pr0->x0, pr1->x0);
        LDRSH    R3,[R1, #+0]
        LDRSH    R4,[R2, #+0]
        CMP      R4,R3
        BGE.N    ??GUI_MergeRect_2
        MOV      R3,R4
??GUI_MergeRect_2:
        STRH     R3,[R0, #+0]
//   50       pDest->y0 = Min(pr0->y0, pr1->y0);
        LDRSH    R3,[R2, #+2]
        LDRSH    R4,[R1, #+2]
        CMP      R3,R4
        BLE.N    ??GUI_MergeRect_3
        MOV      R3,R4
??GUI_MergeRect_3:
        STRH     R3,[R0, #+2]
//   51       pDest->x1 = Max(pr0->x1, pr1->x1);
        LDRSH    R3,[R2, #+4]
        LDRSH    R4,[R1, #+4]
        CMP      R3,R4
        BGE.N    ??GUI_MergeRect_4
        MOV      R3,R4
??GUI_MergeRect_4:
        STRH     R3,[R0, #+4]
//   52       pDest->y1 = Max(pr0->y1, pr1->y1);
        LDRSH    R3,[R2, #+6]
        LDRSH    R4,[R1, #+6]
        CMP      R3,R4
        BGE.N    ??GUI_MergeRect_5
        MOV      R3,R4
??GUI_MergeRect_5:
        STRH     R3,[R0, #+6]
//   53       return;
        B.N      ??GUI_MergeRect_6
//   54     }
//   55     *pDest = *(pr0 ? pr0 : pr1);
??GUI_MergeRect_1:
        CMP      R1,#+0
        BNE.N    ??GUI_MergeRect_7
        MOV      R1,R2
??GUI_MergeRect_7:
        MOVS     R2,#+8
        POP      {R3,R4}
