...
//    Endian       =  little
//    Source file  =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_¼ÎºÌÈýÎ¬\mksRobin_Nano_code\Middlewares\Ucgui\GUI\WM\WM_Show.c
//    Command line =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_¼ÎºÌÈýÎ¬\mksRobin_Nano_code\Middlewares\Ucgui\GUI\WM\WM_Show.c
...
//    List file    =  
//        E:\code\Robin_Nano_SVN\branches\robin_nano_tft35_¼ÎºÌÈýÎ¬\mksRobin_Nano_code\EWARM\mksRobin2/List\WM_Show.s
//
///////////////////////////////////////////////////////////////////////////////
        ...
        PUBLIC WM_InvalidateWindowDescs
        PUBLIC WM_ShowWindow
...
//   32 /*********************************************************************
//   33 *
//   34 *       WM_InvalidateWindowDescs
//   35 
//   36   Invalidate window and all descendents (children and grandchildren and ...
//   37 */

        SECTION `.text`:CODE:NOROOT(1)
          CFI Block cfiBlock0 Using cfiCommon0
          CFI Function WM_InvalidateWindowDescs
        THUMB
//   38 void WM_InvalidateWindowDescs(WM_HWIN hWin) {
...
//   39   WM_HWIN hChild;
//   40   if (hWin) {
        BEQ.N    ??WM_InvalidateWindowDescs_0
//   41     WM_InvalidateWindow(hWin);    /* Invalidate window itself */
          CFI FunCall WM_InvalidateWindow
        BL       WM_InvalidateWindow
//   42     for (hChild = WM_GetFirstChild(hWin); hChild;) {
        MOV      R0,R4
          CFI FunCall WM_GetFirstChild
        BL       WM_GetFirstChild
        MOV      R4,R0
        B.N      ??WM_InvalidateWindowDescs_1
//   43       WM_Obj* pChild = WM_H2P(hChild);
??WM_InvalidateWindowDescs_2:
        MOV      R0,R4
          CFI FunCall GUI_ALLOC_h2p
        BL       GUI_ALLOC_h2p
        MOV      R5,R0
//   44       WM_InvalidateWindowDescs(hChild);
        MOV      R0,R4
          CFI FunCall WM_InvalidateWindowDescs
        BL       WM_InvalidateWindowDescs
//   45       hChild = pChild->hNext;
        LDRSH    R4,[R5, #+26]
//   46     }
??WM_InvalidateWindowDescs_1:
        CMP      R4,#+0
        BNE.N    ??WM_InvalidateWindowDescs_2
//   47   }
//   48 }
??WM_InvalidateWindowDescs_0:
...
//   53 */
        ...
        SECTION `.text`:CODE:NOROOT(1)
          ...
          CFI Function WM_ShowWindow
        THUMB
//   54 void WM_ShowWindow(WM_HWIN hWin) {
WM_ShowWindow:
        PUSH     {R4,LR}
          CFI R14 Frame(CFA, -4)
          CFI R4 Frame(CFA, -8)
        ...
        MOVS     R4,R0
//   55   if (hWin) {
        BEQ.N    ??WM_ShowWindow_0
//   56     WM_Obj *pWin;
//   57     WM_LOCK();
//   58     pWin = WM_H2P(hWin);  
          CFI FunCall GUI_ALLOC_h2p
        BL       GUI_ALLOC_h2p
//   59     if ((pWin->Status & WM_SF_ISVIS) == 0) {  /* First check if this is necessary at all */
        LDRH     R1,[R0, #+28]
        LSLS     R2,R1,#+30
        BMI.N    ??WM_ShowWindow_0
//   60       pWin->Status |= WM_SF_ISVIS;  /* Set Visibility flag */
        ORR      R1,R1,#0x2
        STRH     R1,[R0, #+28]
//   61       WM_InvalidateWindowDescs(hWin);    /* Mark content as invalid */
        MOV      R0,R4
        POP      {R4,LR}
          CFI R4 SameValue
          CFI R14 SameValue
          CFI CFA R13+0
          CFI FunCall WM_InvalidateWindowDescs
        B.N      WM_InvalidateWindowDescs
          CFI R4 Frame(CFA, -8)
          CFI R14 Frame(CFA, -4)
          CFI CFA R13+8
//   62       #if WM_SUPPORT_NOTIFY_VIS_CHANGED
//   63         WM__NotifyVisChanged(hWin, &pWin->Rect);
//   64       #endif
//   65     }
//   66     WM_UNLOCK();
//   67   }
...
//   69 
//   70 #else
//   71   void WM_Show_c(void) {} /* avoid empty object files */
