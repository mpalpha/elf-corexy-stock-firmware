...
static BUTTON_STRUCT buttonMoto_off,buttonLed;
...
//	int imgHeight = LCD_HEIGHT - titleHeight;	
	...
	if(gCfgItems.enable_off ==1)
	{
		buttonFunction_1.btnHandle = BUTTON_CreateEx(INTERVAL_V, 0,BTN_X_PIXEL, BTN_Y_PIXEL, hSetWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
		...
		if(gCfgItems.display_style == 0)
		{
			...
			buttonLed.btnHandle = BUTTON_CreateEx(BTN_X_PIXEL+INTERVAL_V*2, BTN_Y_PIXEL+INTERVAL_H,BTN_X_PIXEL, BTN_Y_PIXEL,hSetWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
	...
	else
	{
		...
		if(gCfgItems.display_style == 0)
		{
			...
			if(gCfgItems.multiple_language !=0)
			{
				buttonLanguage.btnHandle  =  BUTTON_CreateEx(INTERVAL_V,  BTN_Y_PIXEL+INTERVAL_H,BTN_X_PIXEL, BTN_Y_PIXEL, hSetWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
				
	                
			}
			if(gCfgItems.enable_led == 1)
			buttonLed.btnHandle = BUTTON_CreateEx(BTN_X_PIXEL*2+INTERVAL_V*3, BTN_Y_PIXEL+INTERVAL_H,BTN_X_PIXEL, BTN_Y_PIXEL,hSetWnd, BUTTON_CF_SHOW, 0, alloc_win_id());//alloc_win_id());
	...
	if(gCfgItems.multiple_language != 0)
	{
		...
		//if(gCfgItems.func_btn1_display_flag != 0)
			BUTTON_SetText(buttonFunction_1.btnHandle, set_menu.shutdown);
        if(gCfgItems.multiple_language !=0)
