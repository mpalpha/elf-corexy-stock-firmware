...
static BUTTON_STRUCT buttonDisableXY;
	...
	switch (pMsg->MsgId)
	{
		...
		case WM_NOTIFY_PARENT:
			if(pMsg->Data.v == WM_NOTIFICATION_RELEASED)
			{
				...
				else if(pMsg->hWinSrc == buttonDisableXY.btnHandle)
				{
					enqueue_and_echo_commands_P(PSTR("M84 X Y"));
				}
...
{	
	...
	buttonDisableXY.btnHandle = BUTTON_CreateEx(BTN_X_PIXEL+INTERVAL_V*2, BTN_Y_PIXEL+INTERVAL_H,BTN_X_PIXEL, BTN_Y_PIXEL, hZeroWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
	BUTTON_SetBmpFileName(buttonDisableXY.btnHandle, "bmp_function1.bin",1);
	BUTTON_SetBitmapEx(buttonDisableXY.btnHandle, 0, &bmp_struct,BMP_PIC_X, BMP_PIC_Y);
...
#if 1
	BUTTON_SetBkColor(buttonDisable.btnHandle, BUTTON_CI_PRESSED, gCfgItems.back_btn_color);
	BUTTON_SetBkColor(buttonDisable.btnHandle, BUTTON_CI_UNPRESSED, gCfgItems.back_btn_color);
	BUTTON_SetTextColor(buttonDisable.btnHandle, BUTTON_CI_PRESSED, gCfgItems.back_btn_textcolor);
	BUTTON_SetTextColor(buttonDisable.btnHandle, BUTTON_CI_UNPRESSED, gCfgItems.back_btn_textcolor);

	BUTTON_SetBkColor(buttonDisableXY.btnHandle, BUTTON_CI_PRESSED, gCfgItems.back_btn_color);
	BUTTON_SetBkColor(buttonDisableXY.btnHandle, BUTTON_CI_UNPRESSED, gCfgItems.back_btn_color);
	BUTTON_SetTextColor(buttonDisableXY.btnHandle, BUTTON_CI_PRESSED, gCfgItems.back_btn_textcolor);
	BUTTON_SetTextColor(buttonDisableXY.btnHandle, BUTTON_CI_UNPRESSED, gCfgItems.back_btn_textcolor);
#endif
	if(gCfgItems.multiple_language != 0)
	{
			...
			BUTTON_SetText(buttonDisableXY.btnHandle, set_menu.motoroffXY);
	}
