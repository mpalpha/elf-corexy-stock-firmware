	...
	if(disp_state == PRINTING_UI)
	{
		...
		if((mksReprint.mks_printer_state == MKS_IDLE)  &&  (rate == 100) )
		{
			...
			{					
				...
				BUTTON_SetBmpFileName(buttonPause.btnHandle,NULL,1);
				BUTTON_SetBitmapEx(buttonPause.btnHandle, 0, &bmp_struct_0_0, BMP_PIC_X, BMP_PIC_Y);
				BUTTON_SetText(buttonPause.btnHandle,"");
				

				BUTTON_SetBmpFileName(buttonStop.btnHandle,NULL,1);
				BUTTON_SetBitmapEx(buttonStop.btnHandle, 0, &bmp_struct_0_0, BMP_PIC_X, BMP_PIC_Y);
				BUTTON_SetText(buttonStop.btnHandle,"");
                #else
				BUTTON_SetBmpFileName(buttonOperat.btnHandle, "bmp_printing_back.bin",1);
				BUTTON_SetBitmapEx(buttonOperat.btnHandle, 0, &bmp_struct_150, BMP_PIC_X, BMP_PIC_Y);
				BUTTON_SetBkColor(buttonOperat.btnHandle, BUTTON_CI_PRESSED,gCfgItems.back_btn_color);
