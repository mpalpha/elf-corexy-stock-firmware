...
static BUTTON_STRUCT buttonFilament,buttonAuto_Off,buttonExtrude,buttonMove,buttonLed;
...
switch (pMsg->MsgId)
{
	...
	case WM_NOTIFY_PARENT:
		...
		if(pMsg->Data.v == WM_NOTIFICATION_RELEASED)
		{
			...
			#endif
			else if(pMsg->hWinSrc == buttonLed.btnHandle)
			{
				if(led_state == 0)
...
void draw_operate()
{
	...
	if((mksReprint.mks_printer_state == MKS_PAUSING)||(mksReprint.mks_printer_state == MKS_PAUSED)||(mksReprint.mks_printer_state ==MKS_REPRINTING)||(mksReprint.mks_printer_state ==MKS_REPRINTED))
	{
		...
		buttonExtrude.btnHandle = BUTTON_CreateEx(BTN_X_PIXEL*3+INTERVAL_V*4,0,BTN_X_PIXEL,BTN_Y_PIXEL,hOperateWnd, BUTTON_CF_SHOW, 0, 106);
		buttonMove.btnHandle = BUTTON_CreateEx(INTERVAL_V,BTN_Y_PIXEL+INTERVAL_H,BTN_X_PIXEL, BTN_Y_PIXEL,hOperateWnd, BUTTON_CF_SHOW, 0, 106);		
		if(gCfgItems.morefunc_cnt > 0)
		buttonMore_printing.btnHandle = BUTTON_CreateEx(BTN_X_PIXEL*2+INTERVAL_V*3, BTN_Y_PIXEL+INTERVAL_H,BTN_X_PIXEL,BTN_Y_PIXEL,hOperateWnd, BUTTON_CF_SHOW, 0, 302);

		if(gCfgItems.print_finish_close_machine_flg == 1)
		{
			buttonAuto_Off.btnHandle = BUTTON_CreateEx(BTN_X_PIXEL+INTERVAL_V*2,BTN_Y_PIXEL+INTERVAL_H,BTN_X_PIXEL,BTN_Y_PIXEL,hOperateWnd, BUTTON_CF_SHOW, 0, 107);
		}
	...
	else //if(mksReprint.mks_printer_state ==MKS_WORKING)
	{
			...
			if(gCfgItems.print_finish_close_machine_flg == 1)
			{
				buttonAuto_Off.btnHandle = BUTTON_CreateEx(INTERVAL_V,BTN_Y_PIXEL+INTERVAL_H,BTN_X_PIXEL,BTN_Y_PIXEL,hOperateWnd, BUTTON_CF_SHOW, 0, 302);
			}
			if(gCfgItems.enable_led == 1)
			buttonLed.btnHandle = BUTTON_CreateEx(BTN_X_PIXEL*2+INTERVAL_V*3, BTN_Y_PIXEL+INTERVAL_H,BTN_X_PIXEL, BTN_Y_PIXEL,hOperateWnd, BUTTON_CF_SHOW, 0, alloc_win_id());
	}
	...
	BUTTON_SetBmpFileName(buttonMore_printing.btnHandle, "bmp_more_opr.bin",1);
	if(gCfgItems.enable_led == 1)
	BUTTON_SetBmpFileName(buttonLed.btnHandle, "bmp_led.bin",1);
	if(gCfgItems.print_finish_close_machine_flg == 1)
	{
		if(IsChooseAutoShutdown == 1)
		{
	...
	BUTTON_SetBitmapEx(buttonMove.btnHandle, 0, &bmp_struct, BMP_PIC_X, BMP_PIC_Y);
	if(gCfgItems.enable_led == 1)
	BUTTON_SetBitmapEx(buttonLed.btnHandle, 0, &bmp_struct,BMP_PIC_X, BMP_PIC_Y);
#if 0
	if(gCfgItems.print_finish_close_machine_flg == 1)
	{
		BUTTON_SetBitmapEx(buttonAuto_Off.btnHandle, 0, &bmp_struct, BMP_PIC_X, BMP_PIC_Y);
	...
	if(gCfgItems.enable_led == 1)
	{
	BUTTON_SetBkColor(buttonLed.btnHandle, BUTTON_CI_UNPRESSED, gCfgItems.btn_color);
	BUTTON_SetBkColor(buttonLed.btnHandle, BUTTON_CI_PRESSED, gCfgItems.btn_color);
	BUTTON_SetTextColor(buttonLed.btnHandle, BUTTON_CI_UNPRESSED, gCfgItems.btn_textcolor);
	BUTTON_SetTextColor(buttonLed.btnHandle, BUTTON_CI_PRESSED, gCfgItems.btn_textcolor);
	}
	if(gCfgItems.multiple_language != 0)
	{
		BUTTON_SetText(buttonFan.btnHandle, operation_menu.fan);
		...
		BUTTON_SetText(buttonRet.btnHandle, common_menu.text_back);
		if(gCfgItems.enable_led == 1)
		BUTTON_SetText(buttonLed.btnHandle, set_menu.LED);
	}
